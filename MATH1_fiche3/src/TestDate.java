public class TestDate{
    ...
    private static boolean estBissextile(int annee){
        return 1901<annee && annee<2099 && annee%4==0 ;
    }
    ...
    private static boolean estBissextile2(int annee){
        return 1600<annee&&annee<3000 && annee%4==0 && annee%100!=0||annee%400==0 ;
    }
      	
    private static boolean estValide(int jour, int mois, int annee){
        boolean a = mois == 4 || mois == 6 || mois == 9 ||mois == 11;
        return jour >= 1 && mois >= 1 && mois<= 12 &&  //limitation globale
                (jour <= 28  && mois == 2 && !estBissextile(annee) //année non bissextile
                        || jour <= 29 && mois == 2 && estBissextile(annee) //année bissextile
                        || jour <= 30 && a //avril-juin-septembre-novembre
                        || jour <= 31 && !a && mois !=2 ); //janvier-mars-mai-juillet-aout-octobre-decembre
    }
    ...
    private static boolean estAnterieure(int jour1, int mois1, int annee1, int jour2, int mois2, int annee2 ){
        return annee1 < annee2 || annee1 == annee2 && (mois1 < mois2 || (mois1 == mois2 && jour1 < jour2))   ;
    }
